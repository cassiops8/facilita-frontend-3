...
function App() {
  ...
  const [viewMode, setViewMode] = useState('normal') // 'normal', 'admin', 'viewing'
  ...
  const handleLogin = (dadosFuncionaria) => {
    ...
    console.log("Dados da funcionária após login:", dadosFuncionaria)
    if (dadosFuncionaria.is_admin) {
      setViewMode('admin')
    }
  }
  ...
  if (viewMode === 'viewing' && funcionariaVisualizando) {
    return (
      <div>
          ...
          <div className="flex justify-between items-center max-w-7xl mx-auto">
            <div>
              <h2 className="text-lg font-semibold text-foreground">
                Visualizando: {funcionariaVisualizando.nome}
              </h2>
              <p className="text-sm text-muted-foreground">
                Modo administrador - Você pode interferir nas conversas
              </p>
            </div>
            <button 
        ...
        <Dashboard 
          funcionaria={funcionariaVisualizando} 
